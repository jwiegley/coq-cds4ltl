...
Lemma law_133 (φ ψ : t) : ψ ⟹ φ U φ.
Lemma law_134 (φ ψ : t) : φ U ψ ⟹ φ ∨ ψ.
Lemma law_135 (φ ψ : t) : φ ∧ ψ ⟹ φ U ψ.
Lemma law_136 (φ ψ : t) : ((φ U ψ) ∨ (φ U ¬ ψ)).
Lemma law_137 (φ ψ : t) : φ ∨ (φ U ψ) ≈ φ ∨ ψ.
Lemma law_138 (φ ψ : t) : (φ U ψ) ∨ ψ ≈ φ U ψ.
Lemma law_139 (φ ψ : t) : (φ U ψ) ∧ ψ ≈ ψ.
Lemma law_140 (φ ψ : t) : (φ U ψ) ∨ (φ ∧ ψ) ≈ φ U ψ.
Lemma law_141 (φ ψ : t) : (φ U ψ) ∧ (φ ∨ ψ) ≈ φ U ψ.
Lemma law_142 (φ ψ : t) : φ U (φ U ψ) ≈ φ U ψ.
Lemma law_143 (φ ψ : t) : (φ U ψ) U ψ ≈ φ U ψ.
Lemma law_144 (φ ψ : t) : φ U ψ ≈ ψ ∨ (φ ∧ ◯ (φ U ψ)).
Lemma law_145 (φ ψ : t) : φ U ψ ≈ (φ ∨ ψ) U ψ.
Lemma law_146 (φ ψ : t) : φ U ψ ≈ (φ ∧ ¬ ψ) U ψ.
Lemma law_147 (φ ψ χ : t) : φ U (ψ ∨ χ) ≈ (φ U ψ) ∨ (φ U χ).
Lemma law_148 (φ ψ χ : t) : ((φ U χ) ∨ (ψ U χ)) ⟹ ((φ ∨ ψ) U χ).
Lemma law_149 (φ ψ χ : t) : (φ ∧ ψ) U χ ≈ (φ U χ) ∧ (ψ U χ).
...
Lemma law_154 (φ ψ : t) : ◯ (φ U ψ) ≈ (◯ φ) U (◯ ψ).
Lemma law_155 (φ ψ : t) : ◇ (φ U ψ) ≉ (◇ φ) U (◇ ψ).
Lemma law_156 (φ : t) : ◇ φ ≈ ⊤ U φ.
Lemma law_160 (φ : t) : φ U □ φ ≈ □ φ.
Lemma law_161 (φ ψ : t) : φ U □ ψ ≈ □ (φ U ψ).
Lemma law_162 (φ ψ : t) : (◇ φ → ((φ → ψ) U φ)).
Lemma law_163 (φ ψ χ : t) : ((φ U ψ) ∧ (¬ ψ U χ)) ⟹ φ U χ.
Lemma law_164 (φ ψ χ : t) : (φ U (ψ U χ)) ⟹ ((φ ∨ ψ) U χ).
Lemma law_165 (φ ψ χ : t) : (φ → ψ) U χ ⟹ ((φ U χ) → (ψ U χ)).
Lemma law_166 (φ ψ χ : t) : ((¬ φ U (ψ U χ)) ∧ (φ U χ)) ⟹ ψ U χ.
Lemma law_167 (φ ψ χ : t) : ((φ U (¬ ψ U χ)) ∧ (ψ U χ)) ⟹ φ U χ.
Lemma law_168 (φ ψ : t) : ((φ U ψ) ∧ (¬ ψ U φ)) ⟹ φ.
Lemma law_169 (φ ψ : t) : (φ ∧ (¬ φ U ψ)) ⟹ ψ.
...
Lemma not_eventually (φ : t) : ¬(◇ φ) ≈ □ ¬φ.
Lemma not_always (φ : t) : ¬(□ φ) ≈ ◇ ¬φ.
Lemma eventually_until (ψ : t) : ◇ ψ ≈ ⊤ U ψ.
Lemma eventually_always_until (φ ψ : t) : ◇ ψ ⟹ □ φ ⟹ φ U ψ.
Lemma always_not_eventually (ψ : t) : □ ψ ≈ ¬◇ ¬ψ.
Lemma next_eventually (φ : t) : ◯ (◇ φ) ≈ ◇ (◯ φ).
Lemma next_always (φ : t) : ◯ (□ φ) ≈ □ (◯ φ).
Lemma until_or (ρ φ ψ : t) : ρ U (φ ∨ ψ) ≈ (ρ U φ) ∨ (ρ U ψ).
Lemma and_until (ρ φ ψ : t) : (φ ∧ ψ) U ρ ≈ (φ U ρ) ∧ (ψ U ρ).
Lemma eventually_or (φ ψ : t) : ◇ (φ ∨ ψ) ≈ (◇ φ) ∨ (◇ ψ).
Lemma always_and (φ ψ : t) : □ (φ ∧ ψ) ≈ (□ φ) ∧ (□ ψ).
Lemma expand_always (φ : t) : □ φ ≈ φ ∧ ◯ (□ φ).
Lemma expand_eventually (φ : t) : ◇ φ ≈ φ ∨ ◯ (◇ φ).
