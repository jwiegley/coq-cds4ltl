...
Axiom (* NEW *) always_until : forall (p q r : t), ¬(⊤ U ¬p) ∧ q U r ≈ (p ∧ q) U (p ∧ r).
...
Theorem (* 4 *) next_or (p q : t) : ◯ (p ∨ q) ≈ ◯ p ∨ ◯ q.
Proof.
  pose proof (next_impl (¬p) q) as H; rewrite not_not in H.
  rewrite H.
  now rewrite <- next_linearity.
Qed.

Theorem (* 5 *) next_and (p q : t) : ◯ (p ∧ q) ≈ ◯ p ∧ ◯ q.
Proof.
...
Theorem (* 6 *) next_eqv (p q : t) : ◯ (p ≡ q) ≈ ◯ p ≡ ◯ q.
Proof.
  rewrite !next_or.
  rewrite !next_not.
...
Theorem (* 20 *) until_true (p : t) : p U ⊤ ≈ ⊤.
Proof.
  rewrite until_expansion.
  now rewrite true_or.
Qed.

Theorem (* 21 *) false_until (p : t) : ⊥ U p ≈ p.
Proof.
  rewrite until_expansion.
  rewrite false_and.
...
Theorem (* 22 *) until_idem (p : t) : p U p ≈ p.
Proof.
  rewrite until_expansion.
  now rewrite or_absorb.
Qed.

Theorem (* 23 *) until_excl_middle (p q : t) : (p U q) ∨ (p U ¬q) ≈ ⊤.
Proof.
...
Theorem (* 24 *) until_24 (p q r : t) : (¬p U (q U r)) ∧ (p U r) ⟹ q U r.
Proof.
...
Theorem (* 25 *) until_25 (p q r : t) : (p U (¬q U r)) ∧ (q U r) ⟹ p U r.
Proof.
  rewrite until_right_or_order.
  rewrite or_comm.
  rewrite until_right_impl.
...
Theorem (* 26 *) until_26 (p q : t) : (p U q) ∧ (¬q U p) ⟹ p.
Proof.
  rewrite until_impl_order.
  now rewrite until_idem.
Qed.

Theorem (* 27 *) until_27 (p q : t) : p ∧ (¬p U q) ⟹ q.
Proof.
  rewrite until_expansion.
  rewrite and_or.
  rewrite <- and_assoc.
...
Theorem (* 28 *) until_28 (p q : t) : p U q ⟹ p ∨ q.
Proof.
  rewrite until_expansion.
  rewrite or_and.
  rewrite and_proj.
...
Theorem (* 29 *) until_insertion (p q : t) : q ⟹ p U q.
Proof.
  rewrite until_expansion.
  now apply or_inj.
Qed.

Theorem (* 30 *) until_30 (p q : t) : p ∧ q ⟹ p U q.
Proof.
  rewrite until_expansion.
  rewrite <- or_inj.
  rewrite and_comm.
...
Theorem (* 31 *) until_absorb_or_u (p q : t) : p ∨ (p U q) ≈ p ∨ q.
Proof.
  split.
...
Theorem (* 32 *) until_absorb_u_or (p q : t) : (p U q) ∨ q ≈ p U q.
Proof.
  rewrite until_expansion.
...
Theorem (* 33 *) until_absorb_u_and (p q : t) : (p U q) ∧ q ≈ q.
Proof.
  split.
  - rewrite and_comm.
    now apply and_proj.
...
Theorem (* 34 *) until_absorb_u_or_and (p q : t) : (p U q) ∨ (p ∧ q) ≈ p U q.
Proof.
  split.
  - rewrite until_30.
    now rewrite or_idem.
...
Theorem (* 35 *) until_absorb_u_and_or (p q : t) : (p U q) ∧ (p ∨ q) ≈ p U q.
Proof.
  split.
  - now apply and_proj.
  - rewrite <- until_28.
...
Theorem (* 36 *) until_left_absorb (p q : t) : p U (p U q) ≈ p U q.
Proof.
  split.
  - rewrite until_right_or_order.
    now rewrite or_idem.
...
Theorem (* 37 *) until_right_absorb (p q : t) : (p U q) U q ≈ p U q.
Proof.
  split.
  - rewrite until_28.
...
Theorem (* NEW *) until_left_and_impl (p q r : t) : p U q ∧ p U r ⟹ p U (q ∨ r).
Proof.
  rewrite until_left_or.
  rewrite and_proj.
  rewrite <- or_inj.
