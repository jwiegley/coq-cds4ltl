...
Definition cong {f : t → t}
           `{@Proper _ (equivalent ==> equivalent)%signature f}
           {x y} (eqv : x ≈ y) : f x ≈ f y.
  now rewrite eqv.
Defined.

Definition icong {f : t → t}
           `{@Proper _ (implies ==> implies)%signature f}
           {x y} (imp : x ⟹ y) : f x ⟹ f y.
  now rewrite imp.
Defined.

Definition icong₂ˡ {f : t → t → t}
           `{@Proper _ (implies ==> implies ==> implies)%signature f}
           {x y z} (imp : x ⟹ y) : f x z ⟹ f y z.
  now rewrite imp.
Defined.

Definition icong₂ʳ {f : t → t → t}
           `{@Proper _ (implies ==> implies ==> implies)%signature f}
           {x y z} (imp : x ⟹ y) : f z x ⟹ f z y.
  now rewrite imp.
Defined.

Definition sym {p q} (pq : p ≈ q) : q ≈ p := symmetry pq.

Definition (* 3 *) next_linearity_new p : ◯ p ≈ ¬◯ ¬p :=
  backward
    ¬◯ ¬p
  ≡⟨⟨ cong (next_not _) ⟩⟩
    ¬¬◯ p
